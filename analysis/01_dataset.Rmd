...
clearance_pivoted_vitals_d <- clearance_pivoted_vitals_d %>%
  rename(heartrate_clear = heartrate,
         respiratoryrate_clear = respiratoryrate,
         spo2_clear = spo2,
         nibp_systolic_clear = nibp_systolic,
         nibp_diastolic_clear = nibp_diastolic,
         nibp_mean_clear = nibp_mean,
         temperature_clear = temperature,
         ibp_systolic_clear = ibp_systolic,
         ibp_diastolic_clear = ibp_diastolic,
         ibp_mean_clear = ibp_mean)
```
...
clearance_pivoted_lab_d <- clearance_pivoted_lab_d %>%
  rename(albumin_clear = albumin,
bilirubin_clear = bilirubin ,
BUN_clear = BUN,
calcium_clear = calcium,
creatinine_clear = creatinine,
glucose_clear = glucose,
bicarbonate_clear = bicarbonate,
TotalCO2_clear = TotalCO2,
hematocrit_clear = hematocrit,
hemoglobin_clear = hemoglobin,
INR_clear = INR,
lactate_clear = lactate,
platelets_clear = platelets,
potassium_clear = potassium,
ptt_clear = ptt,
sodium_clear = sodium,
wbc_clear = wbc,
bands_clear = bands,
...
final_df <- final_df %>%
  mutate(
    albumin_clear = ifelse(is.na(albumin_clear), median(albumin_clear, na.rm = TRUE), albumin_clear),
    bilirubin_clear = ifelse(is.na(bilirubin_clear), median(bilirubin_clear, na.rm = TRUE), bilirubin_clear),
    BUN_clear = ifelse(is.na(BUN_clear), median(BUN_clear, na.rm = TRUE), BUN_clear),
    calcium_clear = ifelse(is.na(calcium_clear), median(calcium_clear, na.rm = TRUE), calcium_clear),
    creatinine_clear = ifelse(is.na(creatinine_clear), median(creatinine_clear, na.rm = TRUE), creatinine_clear),
    glucose_clear = ifelse(is.na(glucose_clear), median(glucose_clear, na.rm = TRUE), glucose_clear),
    bicarbonate_clear = ifelse(is.na(bicarbonate_clear), median(bicarbonate_clear, na.rm = TRUE), bicarbonate_clear),
    TotalCO2_clear = ifelse(is.na(TotalCO2_clear), median(TotalCO2_clear, na.rm = TRUE), TotalCO2_clear),
    hematocrit_clear = ifelse(is.na(hematocrit_clear), median(hematocrit_clear, na.rm = TRUE), hematocrit_clear),
    hemoglobin_clear = ifelse(is.na(hemoglobin_clear), median(hemoglobin_clear, na.rm = TRUE), hemoglobin_clear),
    INR_clear = ifelse(is.na(INR_clear), median(INR_clear, na.rm = TRUE), INR_clear),
    lactate_clear = ifelse(is.na(lactate_clear), median(lactate_clear, na.rm = TRUE), lactate_clear),
    platelets_clear = ifelse(is.na(platelets_clear), median(platelets_clear, na.rm = TRUE), platelets_clear),
    potassium_clear = ifelse(is.na(potassium_clear), median(potassium_clear, na.rm = TRUE), potassium_clear),
    ptt_clear = ifelse(is.na(ptt_clear), median(ptt_clear, na.rm = TRUE), ptt_clear),
    sodium_clear = ifelse(is.na(sodium_clear), median(sodium_clear, na.rm = TRUE), sodium_clear),
    wbc_clear = ifelse(is.na(wbc_clear), median(wbc_clear, na.rm = TRUE), wbc_clear),
    bands_clear = ifelse(is.na(bands_clear), median(bands_clear, na.rm = TRUE), bands_clear),
    alt_clear = ifelse(is.na(alt_clear), median(alt_clear, na.rm = TRUE), alt_clear),
    ast_clear = ifelse(is.na(ast_clear), median(ast_clear, na.rm = TRUE), ast_clear),
    alp_clear = ifelse(is.na(alp_clear), median(alp_clear, na.rm = TRUE), alp_clear)
  )
```
...
final_df <- final_df %>%
  mutate(
    heartrate_clear = ifelse(is.na(heartrate_clear), median(heartrate_clear, na.rm = TRUE), heartrate_clear),
    respiratoryrate_clear = ifelse(is.na(respiratoryrate_clear), median(respiratoryrate_clear, na.rm = TRUE), respiratoryrate_clear),
    spo2_clear = ifelse(is.na(spo2_clear), median(spo2_clear, na.rm = TRUE), spo2_clear),
    nibp_systolic_clear = ifelse(is.na(nibp_systolic_clear), median(nibp_systolic_clear, na.rm = TRUE), nibp_systolic_clear),
    nibp_diastolic_clear = ifelse(is.na(nibp_diastolic_clear), median(nibp_diastolic_clear, na.rm = TRUE), nibp_diastolic_clear),
    nibp_mean_clear = ifelse(is.na(nibp_mean_clear), median(nibp_mean_clear, na.rm = TRUE), nibp_mean_clear),
    temperature_clear = ifelse(is.na(temperature_clear), median(temperature_clear, na.rm = TRUE), temperature_clear),
    ibp_systolic_clear = ifelse(is.na(ibp_systolic_clear), median(ibp_systolic_clear, na.rm = TRUE), ibp_systolic_clear),
    ibp_diastolic_clear = ifelse(is.na(ibp_diastolic_clear), median(ibp_diastolic_clear, na.rm = TRUE), ibp_diastolic_clear),
    ibp_mean_clear = ifelse(is.na(ibp_mean_clear), median(ibp_mean_clear, na.rm = TRUE), ibp_mean_clear)
  )
```
